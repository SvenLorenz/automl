...
class NASOptimizer(object):
    ...
    def train_and_eval(self, config: CS.Configuration):
        ...
        better = y < self.curr_incumbent_error
        # TODO: updated the incumbent trajectory
        self.curr_wallclock += 1
        if better:
            self.incumbent_trajectory_error.append(y)
            ...
            self.incumbent_trajectory.append(self.incumbent_trajectory[-1])
    ...
    def optimize(
        ...
        while len(self.history) < n_iters:
            ...
            if regularized:
                ...
                worst_config_idx = np.argmax(
                    [self.train_and_eval(config)[0] for config in self.population]
                )
                worst_config = self.population[worst_config_idx]
                self.population.remove(worst_config)
