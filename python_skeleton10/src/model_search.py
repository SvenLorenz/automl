...
from utils import Conv, FactorizedReduce, Identity, ReLUConvBN
...
class MixedOp(nn.Module):
    ...
    def forward(self, x, alphas):
        ...
        soft_alphas = F.softmax(alphas.reshape(-1))
        list_of_tensors = []
        ...
        for i, op in enumerate(self._ops):
            ...
            list_of_tensors.append(op(x) * soft_alphas[i])
...
class Network(nn.Module):
    ...
    def __init__(self, device, nodes=2):
        ...
        reduction_cell_1 = Cell(nodes, 1, 16, reduction=True)
        normal_cell = Cell(nodes, 2 * 16, 16, reduction=False)
        reduction_cell_2 = Cell(nodes, 2 * 16, 32, reduction=True)

        self.cells = nn.ModuleList(
            [reduction_cell_1, normal_cell, reduction_cell_2])
    ...
    def _initialize_alphas(self, device):
        ...
        self.alphas_normal = Variable(
            1e-3 * torch.randn(k, num_ops, device=device), requires_grad=True
        )
    ...
    def genotype(self):
        ...
        def _parse(alphas):
            ...
            for i in range(self._nodes):
                ...
                edges = sorted(
                    range(i + 1), key=lambda x: -max(W[x][k] for k in range(len(W[x])))
                )[:2]
                for j in edges:
                    k_best = None
